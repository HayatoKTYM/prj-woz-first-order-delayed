...
def main():
    ...
    for phase in df_dict.keys():
        ...
        for i in range(len(df)):
            ...
            print('ターン取得数の合計は，{}'.format(y.sum()))
            y[0] = 0.0  # 最初の start アクションは予測する必要なし
            # sysの発話中も u(t) = 0 にする
            action_c = df[i]['action'].map(lambda x: 1 if x in ['Active-Continue','Passive-Continue'] else 0)
            if phase == 'train':
                u[action_c == 1] = 0.
            # u(t) = 0 の区間は y=0 にする (turn交代の被り)
            y[u == 0] = 0.
            print('ターン被りのない発話の合計は，{}'.format(y.sum()))
